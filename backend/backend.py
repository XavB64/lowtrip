...
def train_to_gdf(
    ...
    if not validate_geom(
        depature_coordinates, arrival_coordinates, gdf.values[0], validate
    ):
        return pd.DataFrame(), False
...
def bus_to_gdf(
    ...
    """
    ### Route OSRM - create a separate function
    geom_route, route_dist, success = find_route(
        departure_coordinates, arrival_coordinates
    )
    ...
    # Validation part for route
    if not validate_geom(
        departure_coordinates, arrival_coordinates, geom_route, validate
    ):
        return pd.DataFrame()
...
def compute_emissions_custom(trip_steps, cmap=cmap_custom):
    ...
    for idx in trip_steps.index[:-1]:  # We loop until last departure
        ...
        if transport_means == "Train":
            gdf, _ = train_to_gdf(
                depature_coords, arrival_coords, colormap=color_custom["Train"]
            )
            l.append(gdf)
            geo.append(gdf)
            ...
            gdf_bus = bus_to_gdf(
                depature_coords, arrival_coords, color=color_custom["Bus"]
            )
            l.append(gdf_bus)
            geo.append(gdf_bus)
            ...
            gdf_car = car_to_gdf(
                depature_coords, arrival_coords, nb=nb, color=color_custom["Car"]
            )
            l.append(gdf_car)
            geo.append(gdf_car)
            ...
            gdf_ferry = ferry_to_gdf(
                depature_coords, arrival_coords, color=color_custom["Ferry"]
            )
            l.append(gdf_ferry)
